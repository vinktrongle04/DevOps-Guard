...
// ─── CONFIGURATION ─────────────────────────────────────────────
// Workspace-aware: if run from monorepo root (no src/, has workspaces),
// auto-detect the first workspace that has a src/ directory.
function resolveScanTarget(targetDir) {
  const pkgPath = path.join(targetDir, 'package.json')
  if (!fs.existsSync(pkgPath)) return { srcDir: path.join(targetDir, 'src'), pkgPath }
  const pkg = JSON.parse(fs.readFileSync(pkgPath, 'utf-8'))
  const hasSrc = fs.existsSync(path.join(targetDir, 'src'))
  if (pkg.workspaces && !hasSrc) {
    const workspaces = Array.isArray(pkg.workspaces) ? pkg.workspaces : []
    for (const ws of workspaces) {
      const wsDir = path.join(targetDir, ws)
      const wsSrc = path.join(wsDir, 'src')
      const wsPkg = path.join(wsDir, 'package.json')
      if (fs.existsSync(wsSrc) && fs.existsSync(wsPkg)) {
        return { srcDir: wsSrc, pkgPath: wsPkg }
      }
    }
  }
