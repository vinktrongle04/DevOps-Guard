...
function fixXSS(lineContent, ruleId) {
  ...
  if (ruleId === 'XSS-002') {
    ...
    result = result.replace(/document\.write\s*\(/, '/* SECURITY: document.write removed */ /*(')
    if (result !== lineContent) return { fixed: true, result, note: 'document.write commented out' }
  }

  if (ruleId === 'XSS-003') {
    // eval() → advisory
    result = result.replace(/\beval\s*\(/, '/* SECURITY: eval() is dangerous */ eval(')
...
function loadViolations() {
  // Run scanner in --json mode to get full violation detail including lineContent
  console.log(C.dim('  Running security scanner in JSON mode...\n'))
  try {
    const output = execSync('node security-scanner.js --json 2>&1', {
      encoding: 'utf-8',
      cwd: __dirname,
    ...
    // exit(1) on violations — stdout still has JSON
    const out = e.stdout || ''
  ...
  if (fs.existsSync(reportPath)) {
    ...
    // Enrich with lineContent by reading source files
    return violations.map(v => ({
      ...v,
      lineContent: readLine(v.file, v.line),
    }))
  }
...
function main() {
  ...
  const filtered = violations.filter(v =>
    !SKIP_PATHS.some(skip => v.file.includes(skip))
  )
